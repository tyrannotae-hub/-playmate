...
export async function POST(
  ...
  // 취소 요청 정보는 RPC가 처리하며 지워버리므로, 알림 메시지에 쓸 정보는 먼저 읽어둔다.
  ...
  const { error } = await supabase.rpc("respond_booking_cancel", {
    p_booking_id: id,
    p_approve: approve,
  });
  ...
  if (before?.parent_id) {
    const message = approve
      ? `${className} 예약 취소 요청이 승인됐어요`
      : `${className} 예약 취소 요청이 거절됐어요`;

    await supabase.from("notifications").insert({
      parent_id: before.parent_id,
      booking_id: id,
      type: approve ? "booking_cancel_approved" : "booking_cancel_rejected",
      message,
    });
    // 알림 생성 실패는 취소 승인/거절 처리 자체를 막지 않음 (에러는 무시)
