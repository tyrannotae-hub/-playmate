...
import { useHideOnScroll, BOTTOM_NAV_TOTAL_HEIGHT } from "@/lib/useHideOnScroll";

// 하단 탭바가 스크롤에 따라 숨었다 나타났다 하는데(useHideOnScroll), 이 CTA는
// 원래 탭바 위에 겹쳐 떠 있는 자리라 탭바가 숨으면 그 빈자리로 같이 내려가야
// 자연스럽다 — 탭바와 같은 훅으로 같은 스크롤 상태를 구독하고, 탭바와 정확히
// 같은 높이(BOTTOM_NAV_TOTAL_HEIGHT)만큼 띄워서 탭바와 겹치지 않게 한다.
  ...
  return (
    <div
      className={`shadow-elevated fixed inset-x-0 z-20 mx-auto w-full max-w-md bg-surface px-4 pt-3 transition-[bottom] duration-300 ${
        navHidden ? "pb-[calc(0.75rem+env(safe-area-inset-bottom))]" : "pb-3"
      }`}
      style={{ bottom: navHidden ? 0 : BOTTOM_NAV_TOTAL_HEIGHT }}
    >
      <Link
