      ...
      <div className="flex gap-2 overflow-x-auto px-4 pb-1">
        {sports.length > 0 && (
          <select
            ...
            className="h-9 flex-shrink-0 rounded-lg border border-line bg-surface px-2.5 text-xs font-bold"
          >
      ...
      <div className="mt-2 flex gap-1.5 overflow-x-auto px-4 pb-1">
        {TIME_SLOTS.map((t) => (
          <button
            key={t}
            onClick={() => setTime(t)}
            className={`h-9 flex-shrink-0 rounded-lg px-3.5 text-xs font-bold transition ${
